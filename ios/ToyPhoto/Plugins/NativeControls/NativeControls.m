...
- (void)showTabBarItems:(NSArray*)arguments withDict:(NSDictionary*)options
{
    ...
    if (!cameraButton) {
        ...
        [cameraButton setBackgroundImage:highlightImage forState:UIControlStateHighlighted];
        ...
        [cameraButton addTarget:self action:@selector(selectCenterButton:) forControlEvents:UIControlEventTouchUpInside];
        [self.webView.superview addSubview:cameraButton];
...
{
    NSString * jsCallBack = [NSString stringWithFormat:@"window.plugins.nativeControls.tabBarItemSelected(%d);", sender.tag];    
    [self.webView stringByEvaluatingJavaScriptFromString:jsCallBack];
}
...
- (void) toolBarButtonTapped:(UIBarButtonItem *)button
{
	...
	for (UIBarButtonItem* currentButton in toolBarItems)
	{
        ...
        if ([button isKindOfClass:[UISegmentedControl class]] ) {
            UIView *view = (UIView*)button;
            if (currentButton.customView == view) {
                NSString * jsCallBack = [NSString stringWithFormat:@"window.plugins.nativeControls.toolBarButtonTapped(%d);", count];    
                [self.webView stringByEvaluatingJavaScriptFromString:jsCallBack];
                return;
            }
